...
def update_old_saves():
    try:
        ...
        with open('save/old_save.json', 'r', encoding='utf-8') as file:
            ...
            updated_saves = []  # Инициализируем переменную
                ...
                if old_save.strip():  # Игнорируем пустые строки
                    try:
                        ...
                        for good_save in good_saves_dicts:
                            ...
                            # Устанавливаем версию из хорошего сохранения
                            if "version" in good_save:
                                updated_character["version"] = int(good_save["version"])
                            #print("МЫ ЗДЕСЬ: updated_character[version']=", updated_character["version"])

                            # Обновляем поля из старого сохранения, если они существуют
                            for key in good_save.keys():
                                if key in dict_character and key != "version":  # Исключаем перезапись версии
                                    updated_character[key] = dict_character[key]
                                    #print(f"Обновлено поле {key}: {updated_character[key]} (из старого: {dict_character[key]})")
                            ...
                            # Добавляем обновленное сохранение в список
