...
class Character:
    ...
    def get_all_params_for_save(self) -> dict:
        save_hero = {
            ...
            'location': self.location.name if isinstance(self.location, Location) else "Неизвестно",
...
class Location:
    def __init__(self, name: str, description: str, danger_level: int) -> None:
        self.name = name
        self.description = description
        self.danger_level = danger_level

    def __str__(self):
        return f"{self.name}: {self.description} (Уровень опасности: {self.danger_level})"
...
#База данных локаций
location_database = [
    Location(name="Зачарованный лес", description="Лес, полный магии и тайн.", danger_level=3),
    Location(name="Безлюдная пустыня", description="Широкие песчаные дюны и отсутствие жизни.", danger_level=2),
    Location(name="Храм", description="Древний храм, хранящий множество секретов.", danger_level=4),
    Location(name="Город", description="Место, полное жизни и возможностей.", danger_level=1),
]

# Создаем экземпляр локации "Храм"
temple_location = Location(name="Храм", description="Древний храм, хранящий множество секретов.", danger_level=4)
...
def generate_inventory(item_database: list, max_item=5):
    ...
    allowed_item_ids = [1, 2, 3, 6, 7, 10]  # Здесь указываем только те id, которые нужны
...
def load_hero_user(*, dict_param: dict, database: list):
    ...
    loading_character_from_file.location = next((loc for loc in location_database if loc.name == location_name), None)
...
def get_player_command():
    ...
    command_options = (
        f"-----------Действия игрока----------\n"
        f"'ГЕРОЙ' или 'Г' характеристики героя и инвентарь \t\tОпыт: {hero_user.experience}/{hero_user.exp_base * 2} \n"
        f"'ПЕРЕМЕЩЕНИЕ' или 'П' для перемещения \t\t\t\t Локация {hero_user.location.name}\n"  # Новая опция
        f"'СОХРАНИТЬ' или 'С' для сохранения прогресса \t\t Здоровье: {hero_user.health_points}/{hero_user.max_health_points()}\n"
        f"'ВЫХОД' или 'В' для выхода\n"
    )

    # Добавляем опцию боя, если не в Городе
    if hero_user.location.name != "Город":
        command_options += (
...
def game() -> None:
    while hero_user.is_alive():
        ...
        if command in ["бой", "б"] and hero_user.location.name != "Город":
            fight_wiht_mob()  # Вызываем бой только если не в Городе
...
while menu not in ["выход", "в"]:
    ...
    if menu in ["старт", "с"]:
        ...
        if hero_user.name.lower() in ["выход", "в"]:
            ...
            continue
        # Создаем экземпляр локации "Храм"
        temple_location = Location(name="Храм", description="Древний храм, хранящий множество секретов.",
                                   danger_level=4)
