...
while menu != "выход" or menu != "в":
    ...
    elif menu == "удалить" or menu == "у":  # Обработка новой команды
        display_saves(get_list_all_saves())  # Показываем сохранения
        save_index = int(input("Введите номер ячейки для удаления: ")) - 1  # Получаем номер ячейки
        delete_specific_save(save_index)  # Удаляем сохранение

    elif menu == "удалить все" or menu == "ув":  # Обработка команды для удаления всех сохранений
        delete_all_saves()
