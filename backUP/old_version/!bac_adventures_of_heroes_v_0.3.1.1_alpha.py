...
class Character:
    ...
    def get_all_params_for_save(self) -> dict:
        save_hero = {
            ...
            'inventory': self.get_list_id_item_from_save(),
            'now_time': round(time.time())
        }
        return save_hero

    def get_list_id_item_from_save(self) -> list:
        i = 0
        inventory_from_save = []
        for _ in self.inventory:
            inventory_from_save.append(self.inventory[i].id_item)
            i += 1
        return inventory_from_save
...
def check_file_save(dict_character:dict) -> bool:
    ...
    if not isinstance(dict_character['count_kill'], int):
        list_error.append("Параметр 'количесвто убийств' не является целым числом")
    ...
    if not isinstance(dict_character['now_time'], int):
        list_error.append("Параметр 'врем сохранения' не является целым числом")
    if len(list_error) > 0:
        for i in list_error:
            print("Ошибка в файле сохранения", i)
        param = False
    else:
        param = True
    return param
