...
def spawn_mob(location: Location):
    if location.name == "Храм":
        level = 1 # Константа! Монстры только первого уровеня
        name = list_name_orcs[0]  # Имя орка из храма (Внизуда)
        allowed_item_ids = [1, 2, 6, 7, 10, 11]  # Предметы, которые могут выпадать от монстров в храме
    elif location.name == "Зачарованный лес":
        level = random.randint(5, 7)
        name = list_name_orcs[2]  # Имя орка из зачарованного леса (Балкмег)
        allowed_item_ids = [1, 2, 3, 9, 4]  # Предметы, которые могут выпадать от монстров в лесу
    elif location.name == "Безлюдная пустыня":
        level = random.randint(10, 13)
        name = list_name_orcs[1]  # Имя орка из пустыни (Азог)
        allowed_item_ids = [2, 3, 5, 8, 9]  # Предметы, которые могут выпадать от монстров в пустыне
    else:
...
def load_hero_user(*, dict_param: dict, database: list):
    ...
    # Проверяем, существует ли локация в базе данных
...
def get_player_command():
    ...
    command_options = (
        ...
        f"'ГЕРОЙ' или 'Г' характеристики героя и инвентарь \t Здоровье: {hero_user.health_points}/{hero_user.max_health_points()} \n"
        f"'ПЕРЕМЕЩЕНИЕ' или 'П' для перемещения \t\t\t\t Опыт: {hero_user.experience}/{hero_user.exp_base * 2}\n"  # Новая опция
        f"'СОХРАНИТЬ' или 'С' для сохранения прогресса \t\t Локация: {hero_user.location.name}\n"
